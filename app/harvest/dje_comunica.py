...
class PublicacaoDJE:
    ...
    polo_advogados: List[dict] = field(default_factory=list)
...
def _extrair_valor_causa(texto: str) -> Optional[float]:
    """Extrai o valor da causa de textos de publicacoes do DJEN."""
    if not texto:
        return None
    m = re.search(
        r"(?:valor\s+da\s+causa|valor\s+causa|valor|importe|quantum|total)"
        r"\s*[:\-\s]*\s*R\$\s*([\d\.]+,\d{2})",
        texto, re.IGNORECASE,
    )
    if m:
...
class PJeComunicaEngine:
    ...
    def fetch_por_cnj(self, numero_cnj: str, days_back: int = 30,
        ...
        pubs, err = self._consulta_publica_cnj(cnj, days_back, tribunais, so_hoje)
        estrategia = "consulta_direta"
        if err:
            ...
            if err2 and self.auth_enabled and self.auth_user:
                _dt_inicio = date.today() if so_hoje else date.today() - timedelta(days=days_back)
                body = {
                    "numeroProcesso": cnj,
                    "dataDisponibilizacaoInicio": _dt_inicio.isoformat(),
    ...
    def _consulta_publica_cnj(self, cnj: str, days_back: int,
                               tribunais: Optional[List[str]] = None,
                               so_hoje: bool = False
                               ) -> Tuple[List[PublicacaoDJE], Optional[str]]:
        """GET /api/v1/comunicacao com filtro por CNJ.

        so_hoje=True: dataInicio == dataFim == hoje.
        """
        data_fim = date.today()
        if so_hoje:
    ...
    def _parse_item_publicacao(self, item) -> Optional[PublicacaoDJE]:
        ...
        classe_nome = item.get("nomeClasse") or item.get("classe_nome") or None
        classe_codigo = (str(item.get("codigoClasse"))
                         if item.get("codigoClasse") is not None else None)
        orgao = (item.get("nomeOrgao") or item.get("orgao")
                 or item.get("nome_orgao") or None)
        tipo_documento = (item.get("tipoDocumento")
                          or item.get("tipo_documento") or None)
        polo_ativo = None
        polo_passivo = None
        polo_advs = []
        for d in dests:
            if not isinstance(d, dict):
                continue
            polo = (d.get("polo") or "").upper().strip()
            nome = (d.get("nome") or "").strip()
            if not nome:
                continue
            if polo.startswith("A") and not polo_ativo:
                polo_ativo = nome
            elif polo.startswith("P") and not polo_passivo:
                polo_passivo = nome
            polo_advs.append({"polo": polo, "nome": nome})
        for a in advs_raw:
            ...
            polo_advs.append({
                "polo": "",
                "nome": adv.get("nome") or "",
                "numero_oab": adv.get("numero_oab"),
                "uf_oab": adv.get("uf_oab"),
            })
        valor_causa = _extrair_valor_causa(item.get("texto") or "")
        return PublicacaoDJE(
            ...
            classe=classe_nome or classe_codigo,
