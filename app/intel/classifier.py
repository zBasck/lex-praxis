...
def classify(texto: str, llm_provider: str = "local", llm_key: str = "",
             llm_model: str = "gpt-4o-mini", timeout: int = 20,
             cfg: dict = None) -> Classificacao:
    """Classifica usando LLM quando disponível e válido; cai para regras se falhar.

    Aceita tanto o estilo antigo (llm_provider/llm_key/llm_model) quanto
    `cfg` (dict com chaves llm_provider/llm_endpoint/llm_model/llm_api_key/
    llm_enabled). Quando cfg vier, ele tem prioridade.
    """
    # Resolve provider/model/key a partir do cfg, se fornecido
    if cfg:
        enabled = cfg.get("llm_enabled")
        if isinstance(enabled, str):
            enabled = enabled.lower() in ("1", "true", "yes", "on")
        if enabled:
            llm_provider = cfg.get("llm_provider") or llm_provider
            llm_model = cfg.get("llm_model") or llm_model
            llm_key = cfg.get("llm_api_key") or llm_key

    # Provider local (Ollama/LM Studio/llama.cpp) - usa LocalLLMClient
    if llm_provider in ("local", "ollama", "openai_compat", "lmstudio"):
        try:
            from app.intel.llm_local import _client_from_cfg, classificar_ato as _classificar
            # Monta cfg a partir do que temos
            local_cfg = cfg or {
                "llm_enabled": True,
            ...
            # Se a engine nao estiver disponivel (Ollama down), cai pra regras
            resp = _classificar(texto or "", local_cfg)
            if resp:
                return Classificacao(
                    tipo_ato=resp,
                    prazo_dias=None,
            ...
            log.warning("LLM local classifier falhou: %s", e)
        return classify_by_rules(texto)

    # Provider pago (OpenAI/Anthropic)
