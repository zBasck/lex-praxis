...
class LocalLLMClient:
    ...
    def is_available(self) -> bool:
        """Retorna True se Ollama esta rodando E o modelo configurado esta instalado.

        Validacao dupla: (1) /api/tags responde 200; (2) o nome do modelo configurado
        bate com algum modelo instalado (com normalizacao de tag, ex: "llama3"
        casa com "llama3:latest"). Sem isso, o sistema pode reportar "disponivel"
        mas toda chamada real retorna 404 do Ollama -- que era o bug quando o
        usuario tinha llm_model=llama3.1:8b no banco mas so llama3:latest instalado.
        """
        if not self.enabled:
            return False
        # Forca IPv4 (127.0.0.1) no lugar de localhost -- Windows as vezes resolve
        # localhost pra ::1 e o Ollama so escuta IPv4.
        endpoint = self.endpoint.replace("localhost", "127.0.0.1") if self.endpoint else self.endpoint
        try:
            if self.provider == "ollama":
                r = requests.get(f"{endpoint}/api/tags", timeout=10)
                if r.status_code != 200:
                    return False
                # Valida se o modelo configurado existe -- evita o caso "Ollama ok
                # mas llm_model=X nao existe e retorna 404 em todo call".
                available = {m.get("name") for m in (r.json() or {}).get("models", [])}
                if self.model:
                    ...
                    if any(m.split(":", 1)[0] == base for m in available):
                        return True
                    log.warning(
                        "LLM: modelo '%s' nao esta instalado no Ollama. "
                        "Disponiveis: %s",
                        self.model, sorted(available) or "(nenhum)",
                    )
                    return False
                return True
            ...
            log.warning("LLM local indisponivel em %s: %s: %s", endpoint, type(e).__name__, e)
        return False
    ...
    def list_models(self) -> list:
        ...
        try:
            ...
            log.warning("Falha listando modelos em %s: %s: %s", endpoint, type(e).__name__, e)
        return []
...
def status(cfg: dict = None) -> dict:
    ...
    erro = None
    if not available:
        # Tenta um probe rapido pra dizer ao usuario o que aconteceu
        try:
            ep = client.endpoint.replace("localhost", "127.0.0.1")
            requests.get(f"{ep}/api/tags" if client.provider == "ollama" else f"{ep}/v1/models", timeout=3)
        except Exception as e:
